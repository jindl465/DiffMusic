...
import time
from transformers_old import get_linear_schedule_with_warmup
from transformers_old import get_constant_schedule_with_warmup
from transformers_old import get_cosine_schedule_with_warmup, AutoProcessor
...
checkpoint_path = f"{PRETRAINED_DIR}/LaDiC.bin"
if os.path.exists(checkpoint_path):
    print(f"기존 체크포인트 {checkpoint_path} 로드 중...")
...
clap_model = clap_model.to(accelerator.device).eval()
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
local_rank = setup_distributed()

train_json_file = '/mnt/storage1/Jin/MUImage/MUImageInstructionsTrain.json'
val_json_file = '/mnt/storage1/Jin/MUImage/MUImageInstructionsTrain.json'
image_dir = '/mnt/storage1/Jin/MUImage/audioset_images'
audio_dir = '/mnt/storage1/Jin/MUImage/audioset'

data_config = {'image_size':224, 'ann_file':train_json_file, 'val_ann_file':val_json_file, 'image_root': image_dir, 'audio_root': audio_dir}
train_set, val_set = create_dataset('muimage', data_config)
...
def get_clap_embeddings(clap_model, text_tokens, audio_waveform):
    with torch.no_grad():
        text_emb = clap_model.get_text_embedding(text_tokens, use_tensor=True)  # (batch, 512)
        audio_emb = clap_model.get_audio_embedding_from_filelist(x = audio_waveform, use_tensor=True)  # (batch, 512)
    return text_emb, audio_emb

def clap_loss_val(text_emb, audio_emb):
    text_emb = F.normalize(text_emb, dim=-1)  
    audio_emb = F.normalize(audio_emb, dim=-1)  
    return 1 - torch.cosine_similarity(text_emb, audio_emb).mean() 
...
def train_func(model, trainer, x, scheduler, train=True):
    ...
    text_emb, audio_emb = get_clap_embeddings(clap_model, x['text'], x['waveform'])
    

    clap_loss_value = clap_loss_val(text_emb, audio_emb)
    clap_loss_value = clap_loss_value.detach()

    l = x_t_loss + x_1_loss + prob_loss + 0.1 * clap_loss_value
    ...
    return l, x_t_loss, x_1_loss, prob_loss, valid_token_loss, pad_loss, clap_loss_value
...
for epoch in range(start_epoch, EPOCH_NUM):
    ...
    for batch_num, x in tqdm(enumerate(train_loader), disable=not accelerator.is_local_main_process):
        l, x_t_loss, x_1_loss, prob_loss, valid_token_loss, pad_loss, clap_loss = train_func(model, optimizer, x, scheduler)

        if batch_num % 50 == 0:
            accelerator.log({
                'loss': l, 'x_t_loss': x_t_loss, 'x_1_loss': x_1_loss,
                'prob_loss': prob_loss, 'valid_token_loss': valid_token_loss,
                'pad_loss': pad_loss, 'clap_loss' : clap_loss
            })
