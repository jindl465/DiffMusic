...
from transformers_old.models.bert.modeling_bert import BertLowModel, BertHighModel
...
from transformers_old import BertTokenizer
...
class Diffuser_with_LN(nn.Module):
    def __init__(self,
                 med_config='/home/cvmlserver10/Jin/diffMusic/LaDiC/configs/med_config.json',
                 image_size=224,
                 vit='base',
                 vit_grad_ckpt=False,
                 vit_ckpt_layer=0,
        ...
        # 기존 Vision Encoder 로드
        self.visual_encoder, _ = load_checkpoint(self.visual_encoder, '/home/cvmlserver10/Jin/diffMusic/LaDiC/pretrained_ckpt/model_base_capfilt_large.pth')

        # 기존 모델 체크포인트 로드
        checkpoint_path = "/home/cvmlserver10/Jin/diffMusic/LaDiC/pretrained_ckpt/LaDiC.bin"
        ...
        lora_config = LoraConfig(
            ...
            target_modules=[
                ...
                "dense",
                "fc1", "fc2",
                "cross_attn.query", "cross_attn.key", "cross_attn.value"
            ],  # Attention Layer만 LoRA 적용 
        )

        self.diffuser = get_peft_model(self.diffuser, lora_config)
        # self.diffuser.load_adapter()
    ...
    def load_visual_encoder_weights(self):
        visual_encoder_state_dict = torch.load('/home/cvmlserver10/Jin/diffMusic/LaDiC/pretrained_ckpt/model_base_capfilt_large.pth', map_location='cpu')['model']
        self.load_state_dict(visual_encoder_state_dict, strict=False)
