...
parser.add_argument(
    '--output_dir', default="/mnt/storage1/Jin/diffMusic/result/test1_muvideo_middle", type=str,
...
def load_model():
    ...
    MODEL_NAME = "/home/cvmlserver10/Jin/diffMusic/LaDiC/checkpoint/maxlen100_epoch100_newmuimage"
...
def main():
    ...
    test_csv_file = "/mnt/storage1/Jin/MUVideo/MUVideoInstructionsEval.json"
    image_dir = "/mnt/storage1/Jin/MUVideo/audioset_video_eval/middle"
    audio_dir = "/mnt/storage1/Jin/MUVideo/audioset_eval"

    config = {"image_size": 224, "test_ann_file": test_csv_file, "image_root": image_dir, "audio_root": audio_dir}
    test_set = create_dataset("muvideo_test", config)
    ...
    if not os.path.exists(args.output_dir+'/llm'):
        os.makedirs(args.output_dir+'/llm', exist_ok=True)
    ...
    for index, result in enumerate(tqdm(generated_captions)):
        ...
        audio_path = image_path.replace(".jpg", ".wav").replace(image_dir, audio_dir)
        ...
        result_audio = audio_path.split("/")[-1]
        
        output_music_path1 = f"{args.output_dir}/llm/{result_audio}"
        generate_music(caption, decoder_model, args.music_decoder, music_length, output_music_path1)

        # Generate expanded music caption
        music_caption = generate_music_caption(image, gt_text, model, gpt2_model, gpt2_tokenizer, bert_tokenizer, gt_text)

        # Generate music
        output_music_path = f"{args.output_dir}/llm/{result_audio}"
        # generate_music(music_caption, decoder_model, args.music_decoder, music_length, output_music_path)

        print(f"Generated Music for {image_path}: {caption}")
        result_json.append({'image_path':image_path, 'gt_text':gt_text, 'generated_caption':caption, 'llm_caption':music_caption})
