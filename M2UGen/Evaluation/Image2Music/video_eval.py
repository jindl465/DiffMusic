...
models = ['diffMusic', 'mozarts', 'codi-v', 'codi-vt']
# models = ['diffMusic']
scores = {model: {"FAD": 0, "CLAP": 0, "IM_RANK": 0} for model in models}
model_files = {"diffMusic": "/mnt/storage1/Jin/diffMusic/result/test1_muvideo_first/llm", "mozarts": "/mnt/storage1/Jin/diffMusic/result/mozarts_video", "codi-v" : "/mnt/storage1/Jin/diffMusic/result/codi_v", "codi-vt": "/mnt/storage1/Jin/diffMusic/result/codi_vt"}
# model_files = {"diffMusic": "/mnt/storage1/Jin/diffMusic/result/test1_muvideo_first/llm"}
...
def imbind_rank(image, m1, m2, m3, c):
    ...
    ranking = torch.argsort(cosine_similarity)
    # print(dot_product)
    # print(ranking)
...
def fad_score(original, generated):
    ...
    # # command = f"fad_embed --verbose vggish {generated}".split(" ")
...
json_data = json.load(open("/mnt/storage1/Jin/MUVideo/MUVideoInstructionsEval.json"))


for model in scores.keys():
    print("fad score start")
    scores[model]["FAD"] = fad_score("/mnt/storage1/Jin/MUVideo/audioset_eval", model_files[model])
    ...
    for music in generated_files:
        p1, p2 = compare_files(f"/mnt/storage1/Jin/MUVideo/audioset_eval/{music}",
...
for row in tqdm(json_data):
    ...
    model_rankings = imbind_rank(image, f"{model_files['diffMusic']}/{music}",
                                 ...
                                 f"{model_files['codi-v']}/{music}",
                                 f"{model_files['codi-vt']}/{music}")
